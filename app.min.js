...
class AppController {
    ...
    initDOMElements() {
        ...
        // Delete marker elements
        this.deleteMarkerBtn = document.getElementById('delete-active-marker-btn');
        this.deleteMarkerModalEl = document.getElementById('deleteMarkerModal');
        this.deleteMarkerConfirm = document.getElementById('confirm-delete-marker-btn');
	...
	// ============================================
        ...
        // Delete marker button and modal (Bootstrap modal)
...
showDeleteMarkerModal() {
    ...
    // Show marker info in modal
    if (this.deleteMarkerInfo) {
        const marker = this.videoController.state.markers[activeIndex];
        const comment = marker.comment || 'No comment';
        this.deleteMarkerInfo.textContent = `Marker ${activeIndex + 1}: "${comment}"`;
    }
...
setupKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
        ...
        switch(e.code) {
            ...
            case 'Escape':
                e.preventDefault();
                // Close delete modal if open (Bootstrap modal check)
...
// Update button states for edit and delete markers
...
renderWaveformMarkerRegions() {
    ...
    // Remove existing marker regions to prevent duplicates
    ...
    this.videoController.state.markers.forEach((marker, index) => {
        const region = this.videoController.wavesurfer.regions.add({
            ...
            attributes: {
                ...
                markerIndex: index,
                markerRef: marker,
                // FIX: Use "data-label" so the CSS attr(data-label) picks it up correctly
                "data-label": `Marker ${index + 1}: ${this.videoController.toSMPTE(marker.start)} - ${this.videoController.toSMPTE(marker.end)}`
            }
        });
