...
class AppController {
    ...
    setupEventListeners() {
        ...
        // OPTIONAL: Allow Enter key in duration inputs to trigger SET
        const triggerSetOnEnter = (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
        ...
        this.minutesInput.addEventListener('keydown', triggerSetOnEnter);
        this.secondsInput.addEventListener('keydown', triggerSetOnEnter);
        this.framesInput.addEventListener('keydown', triggerSetOnEnter);
        ...
        // FIXED: These were incorrectly inside framerate handler - now at END of setupEventListeners
...
addMarker() {
	...
	// STEP 4: Scroll to the marker at its NEW position
	...
	// STEP 5: Scroll to marker
	setTimeout(() => {
		const markerElement = document.querySelector(`.marker-item[data-index="${newMarkerIndex}"]`);
		if (markerElement) {
			markerElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
		}
	}, 100);
}
...
// ============================================ 
// MISSING METHOD: setupWaveformEditModeHandlers (FIXED) 
// ============================================ 
setupWaveformEditModeHandlers() {
    if (!this.videoController.wavesurfer || !this.videoController.wavesurfer.regions) {
        return;
    }
    ...
    waveformContainer.addEventListener('contextmenu', (e) => {
        ...
        Object.values(regions).forEach(region => {
            if (region.attributes && region.attributes.isMarker) {
                if (region.element && region.element.contains(e.target)) {
                    clickedIndex = region.attributes.markerIndex;
                }
            }
        });
        ...
        if (clickedIndex !== -1) {
            e.preventDefault();
            this.showMarkerContextMenu(e, clickedIndex);
        }
    });
}

showMarkerContextMenu(event, markerIndex) {
    document.querySelectorAll('.waveform-context-menu').forEach(m => m.remove());
    ...
    menu.innerHTML = `
        <div class="context-menu-item" data-action="load">Load Marker</div>
        <div class="context-menu-item" data-action="edit">Edit Comment</div>
        <hr>
        <div class="context-menu-item" data-action="delete">Delete Marker</div>
    `;
    ...
    menu.querySelectorAll('.context-menu-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (action === 'load') {
                ...
                const markerElement = document.querySelector(`.marker-item[data-index="${markerIndex}"]`);
                if (markerElement) {
                    this.editMarkerText(markerIndex, markerElement);
                    document.getElementById('markers-tab').click();
                }
	...
	this.videoController.state.markers.forEach((marker, index) => {
		const region = this.videoController.wavesurfer.regions.add({
			...
			attributes: {
				isMarker: true,
				markerIndex: index,
				markerRef: marker,  // ADD THIS LINE - stores object reference
				label: `Marker ${index + 1}`
			}
		});
		
		this.attachRegionEventListeners(region, index, marker);  // ADD marker parameter
	});	
}
...
attachRegionEventListeners(region, initialIndex, markerRef) {
	...
	region.on('update-end', () => {
		...
		// Update comments
		if (markerRef.comments && markerRef.comments.length > 0) {
			const frameRate = this.videoController.state.frameRate;
			markerRef.comments[0].start = Math.round(markerRef.start * frameRate);
			markerRef.comments[0].end = Math.round(markerRef.end * frameRate);
		}
	...
	region.on('dblclick', (e) => {
		...
		const currentIndex = this.videoController.state.markers.indexOf(markerRef);
		if (currentIndex !== -1) {
			if (confirm(`Delete Marker ${currentIndex + 1}?`)) {
				this.removeMarker(currentIndex);
			}
		}
	});
