...
renderWaveformMarkerRegions() {
    ...
    this.videoController.state.markers.forEach((marker, index) => {
        ...
        // Add comment label to region element
        if (region.element && labelText) {
            ...
            // Update label when region becomes active
            const updateLabelStyle = () => {
                const isActive = this.videoController.state.activeMarkerIndex === index;
                if (isActive) {
                    labelElement.style.background = 'rgba(255, 215, 0, 0.9)';
                } else {
            ...
            region.updateLabelStyle = updateLabelStyle;
        }
...
createMarkerFromSubtitle(subtitle) {
    ...
    // Add these lines to update all displays:
    this.updateMarkersList();
    this.renderTimelineMarkers();
    this.renderWaveformMarkerRegions();
    ...
    // Optional: Scroll to the new marker
    setTimeout(() => {
        const markerIndex = this.videoController.state.markers.length - 1;
