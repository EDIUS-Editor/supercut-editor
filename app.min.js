...
handleTimeUpdate() {
    ...
    // Play Regions mode
    if (window.app && window.app.handlePlayRegionsTimeUpdate) {
        window.app.handlePlayRegionsTimeUpdate();
    }
}
...
class AppController {
    ...
    initDOMElements() {
        ...
        this.editActiveMarkerBtn = document.getElementById('edit-active-marker');
        ...
        this.setPreciseRangeBtn = document.getElementById('set-precise-range');
        ...
        // Delete marker elements — FIXED IDs to match HTML
        this.deleteMarkerBtn = document.getElementById('delete-active-marker');
        this.deleteMarkerModalEl = document.getElementById('deleteMarkerModal');
        this.deleteMarkerConfirm = document.getElementById('confirm-delete-btn');
        this.deleteMarkerInfo = document.getElementById('delete-marker-info');
        
        // Toggle subtitles
        this.toggleSubtitlesBtn = document.getElementById('toggle-subtitles');
        
        // Play regions
        this.playRegionsBtn = document.getElementById('play-regions');
        
        // Bulk delete marker elements
        this.bulkDeleteMarkerModalEl = document.getElementById('bulkDeleteMarkerModal');
        this.bulkDeleteTitle = document.getElementById('bulk-delete-title');
        this.bulkDeleteInfo = document.getElementById('bulk-delete-info');
        this.confirmBulkDeleteBtn = document.getElementById('confirm-bulk-delete-btn');
        this.markerSearchActions = document.getElementById('marker-search-actions');
        this.markerMatchCounter = document.getElementById('marker-match-counter');
        this.keepOnlyMatchesBtn = document.getElementById('keep-only-matches');
        this.deleteMatchesBtn = document.getElementById('delete-matches');
    }
    ...
    init() {
        ...
        if (this.deleteMarkerModalEl) {
            this.deleteMarkerModal = new bootstrap.Modal(this.deleteMarkerModalEl);
        }
        if (this.bulkDeleteMarkerModalEl) {
            this.bulkDeleteMarkerModal = new bootstrap.Modal(this.bulkDeleteMarkerModalEl);
        }
    }
    
    setupEventListeners() {
        // Playback controls — modified to handle Play Regions
        this.playPauseBtn.addEventListener('click', () => {
            // Stop regions mode if user manually pauses while playing
            if (this.state.isPlayingRegions && !this.video.paused) {
                this.stopPlayRegions();
            }
            this.videoController.playPause();
        });
        
        this.stopBtn.addEventListener('click', () => {
            if (this.state.isPlayingRegions) this.stopPlayRegions();
        ...
        this.toggleSubtitlesBtn.addEventListener('click', () => {
            this.toggleSubtitlesBtn.classList.toggle('active');
            this.currentSubtitleDisplay.classList.toggle('hidden');
        });
        
        // Play Regions toggle
        this.playRegionsBtn.addEventListener('click', () => {
            if (this.state.isPlayingRegions) {
                this.videoController.pause();
                this.stopPlayRegions();
            } else {
                this.startPlayRegions();
            }
        });
        ...
        // Delete marker button and modal
        if (this.deleteMarkerBtn) {
            this.deleteMarkerBtn.addEventListener('click', () => this.showDeleteMarkerModal());
        }
        if (this.deleteMarkerConfirm) {
            this.deleteMarkerConfirm.addEventListener('click', () => this.confirmDeleteActiveMarker());
        }
        ...
        // Bulk delete marker actions
        this.keepOnlyMatchesBtn.addEventListener('click', () => this.showBulkDeleteConfirmation('keep'));
        this.deleteMatchesBtn.addEventListener('click', () => this.showBulkDeleteConfirmation('delete'));
        this.confirmBulkDeleteBtn.addEventListener('click', () => this.executeBulkDelete());
...
// ============================================ 
startPlayRegions() {
    if (!this.videoController.state.markers.length || !this.video.duration) return;

    this.state.isPlayingRegions = true;
    this.state.playingRegionIndex = 0;
    this.playRegionsBtn.classList.add('active');
    this.playRegionsBtn.innerHTML = '\u23f9 Regions';

    // Jump to the first marker and play
    const firstMarker = this.videoController.state.markers[0];
    this.videoController.seekTo(firstMarker.start);
    this.videoController.play();
...
stopPlayRegions() {
    this.state.isPlayingRegions = false;
    this.state.playingRegionIndex = -1;
    this.playRegionsBtn.classList.remove('active');
    this.playRegionsBtn.innerHTML = '\u25b6 Regions';
}

handlePlayRegionsTimeUpdate() {
    if (!this.state.isPlayingRegions || this.state.playingRegionIndex === -1) return;

    const currentMarker = this.videoController.state.markers[this.state.playingRegionIndex];
    if (!currentMarker) {
        this.stopPlayRegions();
        return;
    }
    ...
    if (this.video.currentTime >= currentMarker.end - 0.05) {
        const nextIndex = this.state.playingRegionIndex + 1;

        if (nextIndex < this.videoController.state.markers.length) {
            // Jump to next marker
            this.state.playingRegionIndex = nextIndex;
            const nextMarker = this.videoController.state.markers[nextIndex];
            this.videoController.seekTo(nextMarker.start);
            this.loadMarker(nextIndex);
        } else {
...
// ============================================ 
// DELETE MARKER MODAL METHODS — FIXED
// ============================================ 
showDeleteMarkerModal() {
    const activeIndex = this.videoController.state.activeMarkerIndex;
    if (activeIndex === -1 || !this.videoController.state.markers[activeIndex]) return;

    const marker = this.videoController.state.markers[activeIndex];
    ...
    const markerNumber = activeIndex + 1;

    if (this.deleteMarkerInfo) {
        this.deleteMarkerInfo.innerHTML = `
            <strong>Marker ${markerNumber}</strong><br>
            Time: ${this.videoController.toSMPTE(marker.start)} - ${this.videoController.toSMPTE(marker.end)}<br>
            Duration: ${this.videoController.toSMPTE(marker.duration)}<br>
            <span class="text-muted small">${displayText.substring(0, 50)}${displayText.length > 50 ? '...' : ''}</span>
        `;
    }

    if (this.deleteMarkerModal) {
...
confirmDeleteActiveMarker() {
    const activeIndex = this.videoController.state.activeMarkerIndex;
    if (activeIndex !== -1 && this.videoController.state.markers[activeIndex]) {
        this.removeMarker(activeIndex);
    }
    this.hideDeleteMarkerModal();
}

// ============================================ 
// BULK MARKER MANAGEMENT
// ============================================ 
getMarkerSearchFilterResults() {
    const searchTerm = this.markerSearchInput.value.toLowerCase().trim();
    const markers = this.videoController.state.markers;
    if (!searchTerm || !markers.length) {
        return { searchTerm: '', matchIndices: [], nonMatchIndices: [], total: markers.length };
    }
...
updateMarkerSearchActions() {
    const { searchTerm, matchIndices, total } = this.getMarkerSearchFilterResults();
    
    if (!searchTerm || total === 0) {
        this.markerSearchActions.style.display = 'none';
        return;
    }
    
    this.markerSearchActions.style.display = 'flex';
    this.markerMatchCounter.textContent = `${matchIndices.length} of ${total} markers found`;
    
    // Disable buttons when they'd have no effect
    this.keepOnlyMatchesBtn.disabled = matchIndices.length === 0 || matchIndices.length === total;
    this.deleteMatchesBtn.disabled = matchIndices.length === 0;
}

showBulkDeleteConfirmation(mode) {
    const { matchIndices, nonMatchIndices, total } = this.getMarkerSearchFilterResults();
    ...
    this.state.pendingBulkDeleteMode = mode;
    this.bulkDeleteTitle.textContent = title;
    this.bulkDeleteInfo.innerHTML = description;
    this.bulkDeleteMarkerModal.show();
}

executeBulkDelete() {
    const mode = this.state.pendingBulkDeleteMode;
    if (!mode) return;
    
    const { matchIndices, nonMatchIndices } = this.getMarkerSearchFilterResults();
    ...
    const newMarkers = this.videoController.state.markers.filter((_, index) => indicesToKeep.has(index));
    
    // Clear active marker (it may have been deleted or its index shifted)
    this.videoController.state.activeMarkerIndex = -1;
    
    // Replace markers
    this.videoController.state.markers = newMarkers;
    
    // Clean up
    this.state.pendingBulkDeleteMode = null;
    this.bulkDeleteMarkerModal.hide();
    
    // Clear search and refresh UI
    this.markerSearchInput.value = '';
    this.updateMarkerSearchClearButton();
    this.updateMarkerButtonStates();
    this.sortMarkers();
...
setupKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
        ...
        switch(e.code) {
            ...
            case 'Delete':
            case 'Backspace':
                if (this.videoController.state.activeMarkerIndex !== -1) {
                    e.preventDefault();
            ...
            case 'Escape':
                e.preventDefault();
                if (this.deleteMarkerModalEl && this.deleteMarkerModalEl.classList.contains('show')) {
                    this.hideDeleteMarkerModal();
                    return;
                }
...
updateMarkerButtonStates() {
    ...
    if (this.deleteMarkerBtn) {
        this.deleteMarkerBtn.disabled = !hasActiveMarker;
    }
}
...
updateMarkersList(searchTerm = '') {
    ...
    this.updateMarkerSearchActions();
}
...
editMarkerText(index, markerElement) {
    ...
    editForm.innerHTML = `
        ...
        <div class="mb-2">
            ...
            <div class="d-flex gap-2">
                <input type="text" class="form-control form-control-sm marker-in-time" value="${this.videoController.toSMPTE(marker.start)}" placeholder="IN: 00:00:00:00">
                <input type="text" class="form-control form-control-sm marker-out-time" value="${this.videoController.toSMPTE(marker.end)}" placeholder="OUT: 00:00:00:00">
            </div>
        </div>
    ...
    editForm.querySelector('.save-edit').addEventListener('click', () => {
        ...
        const newInTime = this.parseSMPTE(editForm.querySelector('.marker-in-time').value);
        const newOutTime = this.parseSMPTE(editForm.querySelector('.marker-out-time').value);
...
parseSMPTE(smpteString) {
    ...
    return hours * 3600 + minutes * 60 + seconds + (frames / this.videoController.state.frameRate);
}
...
parseSRT(srtContent) {
    ...
    subtitleBlocks.forEach(block => {
        ...
        srtItems.push({ startTime, endTime, text, startTimeString: this.formatTimeForDisplay(startTime) });
    });
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('-->')) {
            if (inCue && currentCueText) {
                vttItems.push({ startTime: currentCueStart, endTime: currentCueEnd, text: currentCueText.trim(), startTimeString: this.formatTimeForDisplay(currentCueStart) });
            }
...
extractWordTimings(vttContent) {
    ...
    for (const line of lines) {
        ...
        for (const match of wordMatches) {
            ...
            if (text) words.push({ text, startTime, endTime });
        }
    }
...
async saveJSON() {
    ...
    const videoData = {
        ...
        "clips": this.videoController.state.markers.map((marker, index) => ({
            ...
            "comments": marker.comments ? marker.comments.map(sub => ({ text: sub.text })) : []
        }))
    };
